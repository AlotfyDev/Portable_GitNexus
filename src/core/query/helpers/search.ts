  ...
  try {
      ...
      try {
        bestChunks = await collectBestChunks(limit, async (fetchLimit) => {
          const vectorQuery = `
            ...
            ORDER BY distance`;
          const embResults = await db.executeQuery(repo.id, vectorQuery);
    ...
    for (const [nodeId, chunk] of Array.from(bestChunks.entries()).slice(0, limit)) {
      ...
      try {
        ...
        const nodeRows = await db.executeParameterized(repo.id, nodeQuery, { nodeId });
...
export async function executeQueryTool(
  ...
  for (const [, item] of merged) {
    ...
    try {
      const cohesionRows = await db.executeParameterized(
        repo.id,
    ...
    if (includeContent) {
      try {
        const contentRows = await db.executeParameterized(
          repo.id,
