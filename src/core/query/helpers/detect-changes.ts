...
export async function executeDetectChanges(
  repo: RepoHandle, params: { scope?: string; base_ref?: string },
): Promise<any> {
  ...
  let diffArgs: string[];
  switch (scope) {
    case 'staged': diffArgs = ['diff', '--staged', '-U0']; break;
    case 'all': diffArgs = ['diff', 'HEAD', '-U0']; break;
    case 'compare':
      if (!params.base_ref) return { error: 'base_ref is required for "compare" scope' };
      diffArgs = ['diff', params.base_ref, '-U0']; break;
    case 'unstaged':
    default: diffArgs = ['diff', '-U0']; break;
  }
  ...
  } catch (err: any) { return { error: `Git diff failed: ${err.message}` }; }
  ...
  for (const fileDiff of fileDiffs) {
    ...
    try {
      const rows = await db.executeParameterized(repo.id, symbolQuery, queryParams);
  ...
  if (changedSymbols.length > 0) {
    ...
    try {
      const procs = await db.executeParameterized(repo.id, `MATCH (n)-[r:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p:Process) WHERE n.id IN $ids RETURN n.id AS nodeId, p.id AS pid, p.heuristicLabel AS label, p.processType AS processType, p.stepCount AS stepCount, r.step AS step`, { ids: symIds });
