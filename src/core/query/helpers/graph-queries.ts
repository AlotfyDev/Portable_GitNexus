...
import { aggregateClusters } from './symbol-resolution.js';
import type { RepoHandle } from './types.js';

const db = createDatabaseProvider();

export async function queryClusters(repo: RepoHandle, limit = 100): Promise<{ clusters: any[] }> {
  try {
...
export async function queryProcesses(repo: RepoHandle, limit = 50): Promise<{ processes: any[] }> {
  try {
    ...
    return {
      processes: processes.map((p: any) => ({
        id: p.id || p[0], label: p.label || p[1],
        heuristicLabel: p.heuristicLabel || p[2], processType: p.processType || p[3],
...
export async function queryClusterDetail(repo: RepoHandle, name: string): Promise<any> {
  const clusters = await db.executeParameterized(
    repo.id,
    `MATCH (c:Community) WHERE c.label = $clusterName OR c.heuristicLabel = $clusterName
     RETURN c.id AS id, c.label AS label, c.heuristicLabel AS heuristicLabel,
            c.cohesion AS cohesion, c.symbolCount AS symbolCount`,
    { clusterName: name },
  );
  ...
  const members = await db.executeParameterized(
    repo.id,
    `MATCH (n)-[:CodeRelation {type: 'MEMBER_OF'}]->(c:Community)
     WHERE c.label = $clusterName OR c.heuristicLabel = $clusterName
     RETURN DISTINCT n.name AS name, labels(n)[0] AS type, n.filePath AS filePath LIMIT 30`,
    { clusterName: name },
  );
...
export async function queryProcessDetail(repo: RepoHandle, name: string): Promise<any> {
  const processes = await db.executeParameterized(
    repo.id,
    `MATCH (p:Process) WHERE p.label = $processName OR p.heuristicLabel = $processName
     RETURN p.id AS id, p.label AS label, p.heuristicLabel AS heuristicLabel,
            p.processType AS processType, p.stepCount AS stepCount LIMIT 1`,
    { processName: name },
  );
  ...
  const steps = await db.executeParameterized(
    repo.id,
    `MATCH (n)-[r:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p {id: $procId})
     RETURN n.name AS name, labels(n)[0] AS type, n.filePath AS filePath, r.step AS step ORDER BY r.step`,
    { procId },
  );

  return {
    process: {
      id: procId, label: proc.label || proc[1],
