...
function hasOrtCudaProvider(): boolean {
  if (getPortability().isPortable) return false;
  try {
...
export class Embedder {
  ...
  constructor(
    ...
    private readonly forceDevice?: Device,
  ) {}
  ...
  getPipeline(): FeatureExtractionPipeline {
    if (!this.pipeline) throw new Error('Embedder not initialized. Call init() first.');
  ...
  async init(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        const devicesToTry: Device[] = this.forceDevice
        ...
        for (const device of devicesToTry) {
          try {
            ...
            this.pipeline = await withHfDownloadRetry(
              () =>
  ...
  async dispose(): Promise<void> {
    if (this.pipeline) {
      try {
        if ('dispose' in this.pipeline && typeof (this.pipeline as any).dispose === 'function') {
 ...
 */
export const getCurrentDevice = (): Device | null => defaultEmbedder?.device ?? null;
...
export const embedText = async (text: string): Promise<Float32Array> => {
  ...
  if (!defaultEmbedder) throw new Error('Embedder not initialized. Call initEmbedder() first.');
