...
function getRequire(): ReturnType<typeof createRequire> {
  if (!getPortability().hasNativeAddons) throw new Error('Native addons unavailable in portable mode');
  if (!_require) _require = createRequire(DEV_APP_ROOT_URL!);
  return _require;
}
...
if (getPortability().hasNativeAddons) {
  try {
    ProtoGrammar = getRequire()('tree-sitter-proto');
  } catch {
...
if (ProtoGrammar) {
  try {
    ...
    const _Parser = getRequire()('tree-sitter') as any;
...
export function extractPackageFromTree(tree: import('web-tree-sitter').Tree): string {
  if (!PACKAGE_PATTERNS) return '';
  for (const match of runCompiledPatterns(PACKAGE_PATTERNS, tree as unknown as import('tree-sitter').Tree)) {
    const pkgNode = match.captures.pkg;
