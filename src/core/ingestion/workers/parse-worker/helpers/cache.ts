import { SyntaxNode, type EnclosingClassInfo, FUNCTION_NODE_TYPES, findEnclosingClassInfo, genericFuncName, inferFunctionLabel, CLASS_CONTAINER_TYPES, findDescendant } from '../../../utils/ast-helpers.js';
import { buildCollisionGroups, typeTagForId, constTagForId, buildMethodProps, arityForIdFromInfo } from '../../../utils/method-props.js';
  ...
  while (current) {
    if (FUNCTION_NODE_TYPES.has(current.type)) {
      ...
      if (funcName) {
        ...
        const encLang = getLanguageFromFilename(filePath) as SupportedLanguages;
    ...
    if (provider.enclosingFunctionFinder) {
      ...
      if (customResult) {
        ...
        let encTypeTag2 = '';
        if (finalLabel === 'Method' || finalLabel === 'Constructor') {
          const encLang2 = getLanguageFromFilename(filePath) as SupportedLanguages;
          const classNode2 =
