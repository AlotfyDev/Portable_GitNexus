...
function getLeiden(): LeidenModule {
  if (_leiden) return _leiden;
  if (!getPortability().hasLeidenAlgorithm) {
    throw new Error('Leiden algorithm unavailable in portable mode');
  }
  const appRoot = fileURLToPath(DEV_APP_ROOT_URL!);
  const _require = createRequire(DEV_APP_ROOT_URL!);
  const leidenPath = resolve(appRoot, 'vendor', 'leiden', 'index.cjs');
  _leiden = _require(leidenPath) as LeidenModule;
  return _leiden;
}
