...
export const pid = <T extends string>(id: T): PipelineId =>
  id as unknown as PipelineId;
...
export type ResourceCheckResult =
  ...
  | { available: false; reason: string; suggest?: string };

export type CachePayload = { type: 'none' } | { type: 'embeddings'; data: { embeddings: unknown[]; totalNodes: number } };
...
export interface PreflightResult {
  ready: boolean;
  fresh: boolean;
  resources: Array<{ id: string; status: ResourceCheckResult }>;
  message?: string;
}
...
export interface PipelineRunReport {
  results: ReadonlyMap<PipelineId, { output: unknown; artifact: ArtifactRecord | undefined; durationMs: number }>;
  skipped: PipelineId[];
  failed: PipelineId[];
  fresh: PipelineId[];
}
