import {
  executeQuery, executeParameterized, isWriteQuery, isLbugReady,
...
export async function executeCypher(repo: RepoHandle, params: { query: string }): Promise<any> {
  if (!isLbugReady(repo.id)) return { error: 'LadybugDB not ready. Index may be corrupted.' };
...
export async function executeExplore(
  repo: RepoHandle, params: { name: string; type: 'symbol' | 'cluster' | 'process' },
): Promise<any> {
  const { name, type } = params;

  if (type === 'symbol') return executeContext(repo, { name });
  ...
  if (type === 'cluster') {
    ...
    return {
      ...
      members: members.map((m: any) => ({ name: m.name || m[0], type: m.type || m[1], filePath: m.filePath || m[2] })),
    };
  }
  ...
  if (type === 'process') {
    const processes = await executeParameterized(
      ...
      `MATCH (p:Process) WHERE p.label = $processName OR p.heuristicLabel = $processName
       RETURN p.id AS id, p.label AS label, p.heuristicLabel AS heuristicLabel, p.processType AS processType, p.stepCount AS stepCount LIMIT 1`,
      { processName: name },
    );
    ...
    return {
      ...
      steps: steps.map((s: any) => ({ step: s.step || s[3], name: s.name || s[0], type: s.type || s[1], filePath: s.filePath || s[2] })),
    };
  }
