...
export async function executeRename(
  repo: RepoHandle, params: {
    symbol_name?: string; symbol_uid?: string; new_name: string;
    file_path?: string; dry_run?: boolean;
  },
): Promise<any> {
  ...
  const lookupResult = await executeContext(repo, { name: params.symbol_name, uid: params.symbol_uid, file_path });
  if (lookupResult.status === 'ambiguous') return lookupResult;
  if (lookupResult.error) return lookupResult;

  const sym = lookupResult.symbol;
  const oldName = sym.name;

  if (oldName === new_name) return { error: 'New name is the same as the current name.' };
  ...
  const addEdit = (filePath: string, line: number, oldText: string, newText: string, confidence: string) => {
    if (!changes.has(filePath)) changes.set(filePath, { file_path: filePath, edits: [] });
  ...
  if (sym.filePath && sym.startLine) {
    try {
      ...
      if (lineIdx >= 0 && lineIdx < lines.length && lines[lineIdx].includes(oldName)) {
        const defRegex = new RegExp(`\\b${escapeRegex(oldName)}\\b`, 'g');
  ...
  const allIncoming = [
    ...(lookupResult.incoming.calls || []), ...(lookupResult.incoming.imports || []),
    ...(lookupResult.incoming.extends || []), ...(lookupResult.incoming.implements || []),
  ];
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(oldName)) {
          addEdit(ref.filePath, i + 1, lines[i].trim(),
            lines[i].replace(new RegExp(`\\b${escapeRegex(oldName)}\\b`, 'g'), new_name).trim(), 'graph');
          graphEdits++;
          break;
        }
      }
  ...
  try {
    ...
    const rgArgs = ['-l', '--type-add', 'code:*.{ts,tsx,js,jsx,py,go,rs,java,c,h,cpp,cc,cxx,hpp,hxx,hh,cs,php,swift}', '-t', 'code', `\\b${oldName}\\b`, '.'];
    const output = execFileSync('rg', rgArgs, { cwd: repo.repoPath, encoding: 'utf-8', timeout: 5000, maxBuffer: 256 * 1024 * 1024 });
  ...
  return {
    status: 'success', old_name: oldName, new_name,
    files_affected: allChanges.length, total_edits: totalEdits,
    graph_edits: graphEdits, text_search_edits: astSearchEdits,
    changes: allChanges, applied: !dry_run,
  };
}
