...
export function ensureStreamIsWritable(res, signal) {
    if (signal?.aborted || res.destroyed || res.writableEnded) {
        throw new ClientDisconnectedError();
    }
}
export async function waitForDrain(res, signal) {
...
export function isClientDisconnectWriteError(err) {
...
export async function writeNdjsonRecord(res, record, signal) {
    ensureStreamIsWritable(res, signal);
    try {
...
function quoteNodeTable(table) {
    return `\`${table.replace(/`/g, '``')}\``;
}
function getNodeQuery(table, includeContent) {
...
function mapGraphNodeRow(table, row, includeContent) {
    return {
        id: row.id ?? row[0],
        label: table,
        properties: {
...
function mapGraphRelationshipRow(row) {
    return {
...
export async function streamGraphNdjson(res, includeContent = false, signal) {
...
export function mountSSEProgress(app, routePath, jm) {
