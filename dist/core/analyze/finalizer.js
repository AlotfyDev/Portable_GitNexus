 ...
 */
export async function countEmbeddings(executeQuery) {
    try {
 ...
 */
export function computeClusterSummary(pipelineResult) {
 ...
 */
export async function buildMeta(input, embeddingCount) {
    const { getRuntimeCapabilities } = await import('../platform/capabilities.js');
    const runtimeCapabilities = getRuntimeCapabilities();
    const effectiveSemanticMode = input.semanticMode ??
    ...
    try {
        ...
        await generateAIContextFiles(input.repoPath, input.storagePath, projectName, {
        ...
        }, undefined, { skipAgentsMd: input.options.skipAgentsMd, noStats: input.options.noStats });
    }
    catch {
