...
export async function runChunkedParseAndResolve(graph, scannedFiles, allPaths, totalFiles, repoPath, pipelineStart, onProgress, options) {
    ...
    if (isDev) {
        ...
        logger.info(`📂 Scan: ${totalFiles} paths, ${totalParseable} parseable (${totalMB.toFixed(0)}MB), ${numChunks} chunks @ ${CHUNK_BYTE_BUDGET / (1024 * 1024)}MB budget`);
    }
    onProgress({
        phase: 'parsing',
        percent: 20,
    ...
    if (!options?.skipWorkers &&
        ...
        try {
            if (!getPortability().hasWorkerPool)
                throw new Error('Worker pool unavailable in portable mode');
            ...
            if (chunkWorkerData) {
                ...
                if (exportedTypeMap.size > 0 && ctx.namedImportMap.size > 0) {
                    ...
                    if (isDev && enrichedCount > 0) {
                        logger.info(`🔗 E1: Seeded ${enrichedCount} cross-file receiver types (chunk ${chunkIdx + 1})`);
                    }
                }
