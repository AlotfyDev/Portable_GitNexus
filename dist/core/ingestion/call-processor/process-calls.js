...
import { FUNCTION_NODE_TYPES, findEnclosingClassId, genericFuncName, } from '../utils/ast-helpers.js';
...
import { buildReceiverTypeIndex, lookupReceiverType, resolveFieldOwnership, } from './helpers/receiver-resolution.js';
...
export const processCalls = async (graph, files, astCache, ctx, onProgress, exportedTypeMap, importedBindingsMap, importedReturnTypesMap, importedRawReturnTypesMap, heritageMap, bindingAccumulator) => {
    ...
    for (let i = 0; i < prepared.length; i++) {
        ...
        const verifiedReceivers = typeEnv.constructorBindings.length > 0
            ? verifyConstructorBindings(typeEnv.constructorBindings, file.path, ctx, undefined, bindingAccumulator)
            : new Map();
        ...
        matches.forEach((match) => {
            ...
            const resolved = resolveCallTarget({
                ...
                receiverTypeName,
                receiverName,
            }, file.path, ctx, undefined, widenCache, undefined, heritageMap, dispatchDecision);
            if (!resolved)
                return;
