...
import { isShortLabel, STRUCTURAL_LABELS, collectBestChunks, } from './types.js';
import { resolveEmbeddingConfig } from './config.js';
import { rankExactEmbeddingRows } from './exact-search.js';
import { EMBEDDING_TABLE_NAME, STALE_HASH_SENTINEL, EMBEDDING_INDEX_NAME, loadVectorExtension, } from '../lbug/repository/embedding-repository.js';
 ...
 */
const queryEmbeddableNodes = async (embeddingRepo) => {
    ...
    return graphNodes.map((gn) => ({
        ...
        endLine: gn.endLine,
        isExported: gn.isExported,
        description: gn.description,
        parameterCount: gn.parameterCount,
        returnType: gn.returnType,
    }));
};
/**
 * Batch INSERT chunk-aware embeddings into CodeEmbedding table
 */
export const batchInsertEmbeddings = async (executorOrRepo, updates) => {
 ...
 */
const createVectorIndex = async (embeddingRepo) => {
    if (!embeddingRepo)
        return false;
 ...
 */
export const runEmbeddingPipeline = async (executeQuery, executeWithReusedStatement, onProgress, config = {}, skipNodeIds, context, existingEmbeddings, embeddingRepo) => {
    const finalConfig = resolveEmbeddingConfig(config);
    let totalChunks = 0;
    const repo = embeddingRepo ?? createEmbeddingRepository(executeQuery, executeWithReusedStatement);
    try {
        ...
        for (let batchIndex = 0; batchIndex < totalNodes; batchIndex += batchSize) {
            ...
            for (const node of batch) {
                ...
                if (isShort) {
                    ...
                    try {
                        ...
                        if (isDev) {
                            logger.warn({ chunkErr }, `⚠️ AST chunking failed for ${node.label} "${node.name}" (${node.filePath}), falling back to character-based chunking:`);
                        }
            ...
            for (let si = 0; si < allTexts.length; si += EMBED_SUB_BATCH) {
                ...
                try {
                    ...
                    logger.error({ embedErr }, `❌ embedBatch failed for ${subTexts.length} texts (first: "${subTexts[0]?.substring(0, 80)}..."):`);
                    throw embedErr;
                }
        ...
        if (isDev) {
            logger.info(`✅ Embedding pipeline complete! (${totalChunks} chunks from ${totalNodes} nodes)`);
        }
        return {
            nodesProcessed: totalNodes,
 ...
 */
export const semanticSearch = async (queryExecutorOrRepo, query, k = 10, maxDistance = 0.5) => {
    if (!isEmbedderReady()) {
    ...
    const embeddingRepo = isRepo ? queryExecutorOrRepo : undefined;
    const executeQuery = isRepo ? undefined : queryExecutorOrRepo;
    if (embeddingRepo ? await embeddingRepo.isVectorIndexReady() : await loadVectorExtension()) {
        try {
            bestChunks = await collectBestChunks(k, async (fetchLimit) => {
                if (embeddingRepo) {
                    const results = await embeddingRepo.semanticSearch(queryVec, fetchLimit, maxDistance);
    ...
    if (bestChunks.size === 0) {
        const embeddingCount = embeddingRepo
            ...
            : await executeQuery(`MATCH (e:${EMBEDDING_TABLE_NAME}) RETURN count(e) AS cnt`).then((countRows) => Number(countRows[0]?.cnt ?? countRows[0]?.[0] ?? 0));
 ...
 */
export const semanticSearchWithContext = async (queryExecutorOrRepo, query, k = 5, _hops = 1) => {
