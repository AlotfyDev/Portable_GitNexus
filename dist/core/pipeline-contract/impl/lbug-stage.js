...
import { initLbug, closeLbug, loadGraphToLbug, } from '../../lbug/lbug-adapter.js';
export function createLadybugStage() {
    return {
        ...
        run: async (ctx) => {
            const ingestion = ctx.results.get(pid(STAGE_IDS.INGESTION));
            if (!ingestion)
                throw new Error('LadybugDB stage requires ingestion results');
            await closeLbug();
            ...
            let lbugMsgCount = 0;
            await loadGraphToLbug(ingestion.graph, ingestion.repoPath, ctx.storagePath, (msg) => {
                lbugMsgCount++;
