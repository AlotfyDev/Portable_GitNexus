...
import { openLbugConnection, isDbBusyError, isOpenRetryExhausted, waitForWindowsHandleRelease, } from '../lbug-config.js';
import { runWithSessionLock, DB_LOCK_RETRY_ATTEMPTS, DB_LOCK_RETRY_DELAY_MS } from './session-lock.js';
import { logger } from '../../logger.js';
export let conn = null;
export let db = null;
export let currentDbPath = null;
let _ftsLoaded = false;
let _vectorExtensionLoaded = false;
export const ensuredFTSIndexes = new Set();
export function getFtsLoaded() { return _ftsLoaded; }
export function setFtsLoaded(v) { _ftsLoaded = v; }
export function getVectorExtensionLoaded() { return _vectorExtensionLoaded; }
export function setVectorExtensionLoaded(v) { _vectorExtensionLoaded = v; }
...
const doInitLbug = async (dbPath) => {
    ...
    try {
        const loaded = await extensionManager.ensure((sql) => conn.query(sql), 'fts', 'FTS', {});
        if (loaded)
            _ftsLoaded = true;
    }
    catch {
