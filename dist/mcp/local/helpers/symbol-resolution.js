...
export async function enrichCandidateLabels(repo, candidates) {
    const ids = candidates.filter((c) => c.type === '' && c.id).map((c) => c.id);
    if (ids.length === 0)
        return;
    try {
        const rows = await executeParameterized(repo.id, `MATCH (n:\`Class\`) WHERE n.id IN $ids RETURN n.id AS id, 'Class' AS label
       ...
       UNION ALL MATCH (n:\`Constructor\`) WHERE n.id IN $ids RETURN n.id AS id, 'Constructor' AS label`, { ids });
...
export function scoreCandidate(c, hints) {
    let s = 0.5;
...
export async function resolveSymbolCandidates(repo, query, hints) {
    ...
    if (uid) {
        ...
        const symbol = {
            ...
            type: (r.type ?? r[2] ?? ''), filePath: (r.filePath ?? r[3]),
            startLine: (r.startLine ?? r[4]), endLine: (r.endLine ?? r[5]),
        ...
        if (ambiguousType) {
            ...
            for (const label of ['Class', 'Interface']) {
                ...
                if (labelRows.length > 0) {
                    ...
                    if (preferred)
                        return { kind: 'ok', symbol: preferred, resolvedLabel: label };
                }
            }
        }
    }
...
export function aggregateClusters(clusters) {
