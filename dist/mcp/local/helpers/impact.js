...
async function _runImpactBFS(repo, sym, symType, direction, opts) {
    ...
    if (symType === 'Class' || symType === 'Interface') {
        try {
            const [ctorRows, fileRows] = await Promise.all([
                executeParameterized(repo.id, `MATCH (n)-[hm:CodeRelation]->(c:Constructor) WHERE n.id = $symId AND hm.type = 'HAS_METHOD' RETURN c.id AS id, c.name AS name, labels(c)[0] AS type, c.filePath AS filePath`, { symId }),
                executeParameterized(repo.id, `MATCH (f:File)-[rel:CodeRelation]->(n) WHERE n.id = $symId AND rel.type = 'DEFINES' RETURN f.id AS id, f.name AS name, labels(f)[0] AS type, f.filePath AS filePath`, { symId }),
            ]);
            ...
            const typedPropertyRows = await executeParameterized(repo.id, `MATCH (p:\`Property\`) WHERE p.declaredType = $name OR p.declaredType STARTS WITH $genericPrefix OR p.declaredType CONTAINS $genericArg RETURN p.id AS id, p.name AS name, labels(p)[0] AS type, p.filePath AS filePath`, { name: sym.name, genericPrefix: `${sym.name}<`, genericArg: `<${sym.name}>` });
    ...
    if (impacted.length > 0) {
        ...
        if (processesMissingMinStep.size > 0) {
            try {
                ...
                const missingRows = await executeParameterized(repo.id, `MATCH (s)-[r:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p:Process) WHERE p.id IN $pIds AND s.id IN $ids RETURN p.id AS pid, MIN(r.step) AS minStep`, { pIds, ids: allImpactedIds }).catch(() => []);
        ...
        const runModuleChunk = async (idsChunk) => {
            if (!idsChunk || idsChunk.length === 0)
                return;
            try {
                const rows = await executeParameterized(repo.id, `MATCH (s)-[:CodeRelation {type: 'MEMBER_OF'}]->(c:Community) WHERE s.id IN $ids RETURN c.heuristicLabel AS name, COUNT(DISTINCT s.id) AS hits ORDER BY hits DESC LIMIT 20`, { ids: idsChunk }).catch(() => []);
                for (const r of rows) {
        ...
        const runDirectModuleChunk = async (idsChunk) => {
            if (!idsChunk || idsChunk.length === 0)
                return;
            try {
                const rows = await executeParameterized(repo.id, `MATCH (s)-[:CodeRelation {type: 'MEMBER_OF'}]->(c:Community) WHERE s.id IN $ids RETURN DISTINCT c.heuristicLabel AS name`, { ids: idsChunk }).catch(() => []);
                for (const r of rows) {
                    const name = r.name ?? r[0] ?? null;
                    if (name)
        ...
        for (let i = 0; i < allIdsArr.length; i += CHUNK_SIZE)
            await runModuleChunk(allIdsArr.slice(i, i + CHUNK_SIZE));
        for (let i = 0; i < d1IdsArr.length; i += CHUNK_SIZE)
            await runDirectModuleChunk(d1IdsArr.slice(i, i + CHUNK_SIZE));
...
export async function executeImpact(repo, params) {
    try {
...
async function _impactImpl(repo, params) {
    ...
    const outcome = await resolveSymbolCandidates(repo, { uid: params.target_uid, name: target }, { file_path: params.file_path, kind: params.kind });
...
export async function executeImpactByUid(ctx, repoId, uid, direction, opts) {
    ...
    let rows;
    try {
        rows = await executeParameterized(repoId, `MATCH (n) WHERE n.id = $uid RETURN n.id AS id, n.name AS name, n.filePath AS filePath, labels(n)[0] AS type LIMIT 1`, { uid });
    }
    catch {
        return null;
    }
