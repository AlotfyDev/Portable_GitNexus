...
export async function dispatchGroupTool(ctx, method, params) {
...
export async function callToolAtGroupRepo(ctx, method, params) {
    ...
    const svc = getGroupService(ctx);
    if (method === 'impact') {
        const impactArgs = { name: groupName, repo: resolved.repoPath, target: params.target, direction: params.direction };
    ...
    if (method === 'query') {
        const queryArgs = { name: groupName, query: params.query };
    ...
    if (method === 'context') {
        ...
        const contextArgs = { name: groupName, target: targetSym };
...
export async function readGroupContractsResource(ctx, groupName, filter) {
    try {
...
export async function readGroupStatusResource(ctx, groupName) {
    try {
...
function formatGroupResourcePayload(raw) {
...
function getGroupService(ctx) {
    return ctx.getGroupService({
        impact: (r, p) => executeImpact(r, p),
        query: (r, p) => executeQueryTool(ctx, r, p),
        impactByUid: (id, uid, d, o) => executeImpactByUid(ctx, id, uid, d, o),
        context: (r, p) => executeContext(r, p),
    });
}
