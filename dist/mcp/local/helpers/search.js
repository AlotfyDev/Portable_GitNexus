...
export async function bm25Search(repo, query, limit) {
    ...
    for (const bm25Result of bm25Results) {
        ...
        try {
            ...
            const symbols = nodeIds
                ? await executeParameterized(repo.id, `MATCH (n) WHERE n.id IN $nodeIds
             RETURN n.id AS id, n.name AS name, labels(n)[0] AS type, n.filePath AS filePath,
                    n.startLine AS startLine, n.endLine AS endLine`, { nodeIds })
                : await executeParameterized(repo.id, `MATCH (n) WHERE n.filePath = $filePath
             RETURN n.id AS id, n.name AS name, labels(n)[0] AS type, n.filePath AS filePath,
                    n.startLine AS startLine, n.endLine AS endLine LIMIT 3`, { filePath: fullPath });
            if (symbols.length > 0) {
...
export async function semanticSearch(ctx, repo, query, limit) {
    try {
        ...
        if (isVectorExtensionSupportedByPlatform()) {
            ...
            logger.warn('GitNexus [query:vector]: VECTOR extension not supported on this platform; using exact scan fallback');
        }
        if (bestChunks.size === 0) {
            ...
            if (embeddingCount > exactLimit)
                return [];
            const rows = await executeQuery(repo.id, `MATCH (e:${EMBEDDING_TABLE_NAME})
...
export async function executeQueryTool(ctx, repo, params) {
    ...
    for (const [, item] of merged) {
        ...
        let processRows = [];
        try {
            processRows = await executeParameterized(repo.id, `MATCH (n {id: $nodeId})-[r:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p:Process)
         RETURN p.id AS pid, p.label AS label, p.heuristicLabel AS heuristicLabel,
                p.processType AS processType, p.stepCount AS stepCount, r.step AS step`, { nodeId: sym.nodeId });
        }
        catch (e) {
        ...
        let module;
        try {
            const cohesionRows = await executeParameterized(repo.id, `MATCH (n {id: $nodeId})-[:CodeRelation {type: 'MEMBER_OF'}]->(c:Community)
         RETURN c.cohesion AS cohesion, c.heuristicLabel AS module LIMIT 1`, { nodeId: sym.nodeId });
        ...
        if (includeContent) {
            try {
                const contentRows = await executeParameterized(repo.id, `MATCH (n {id: $nodeId}) RETURN n.content AS content`, { nodeId: sym.nodeId });
    ...
    const processSymbols = rankedProcesses.flatMap((p) => p.symbols.slice(0, maxSymbolsPerProcess).map((s) => ({ ...s })));
    const seen = new Set();
