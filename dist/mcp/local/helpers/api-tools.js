...
async function fetchRoutesWithConsumers(repoId, routeFilter, params) {
    const rows = await executeParameterized(repoId, `MATCH (n:Route) WHERE n.id STARTS WITH 'Route:' ${routeFilter}
     ...
     RETURN n.id AS routeId, n.name AS routeName, n.filePath AS handlerFile,
            ...
            consumer.name AS consumerName, consumer.filePath AS consumerFile, r.reason AS fetchReason`, params);
...
async function fetchLinkedFlowsBatch(repoId, nodeIds) {
    const result = new Map();
    if (nodeIds.length === 0)
        return result;
    try {
        const rows = await executeParameterized(repoId, `MATCH (source)-[r:CodeRelation]->(proc:Process)
       WHERE r.type = 'ENTRY_POINT_OF' AND list_contains($nodeIds, source.id)
       RETURN source.id AS sourceId, proc.label AS name`, { nodeIds });
        for (const row of rows) {
...
export async function executeRouteMap(repo, params) {
...
export async function executeShapeCheck(repo, params) {
    const routeFilter = params.route ? `AND n.name CONTAINS $route` : '';
    const queryParams = params.route ? { route: params.route } : {};
    ...
    const results = allRoutes
        ...
        return { route: r.name, handler: r.filePath, ...(responseKeys.length > 0 ? { responseKeys } : {}), ...(errorKeys.length > 0 ? { errorKeys } : {}), consumers, ...(hasMismatches ? { status: 'MISMATCH' } : {}) };
    });
...
export async function executeToolMap(repo, params) {
    const toolFilter = params.tool ? `AND n.name CONTAINS $tool` : '';
    const queryParams = params.tool ? { tool: params.tool } : {};
    const rows = await executeParameterized(repo.id, `MATCH (n:Tool) WHERE n.id STARTS WITH 'Tool:' ${toolFilter}
     RETURN n.id AS id, n.name AS name, n.filePath AS filePath, n.description AS description`, queryParams);
    if (rows.length === 0) {
...
export async function executeApiImpact(repo, params) {
    ...
    const results = routes.map((r) => {
        ...
        return {
            ...
            ...(middlewarePartial ? { middlewareDetection: 'partial', middlewareNote: 'Middleware captured from first HTTP method export only — other methods in this handler may use different middleware chains.' } : {}),
            consumers,
