...
import { initLbug, closeLbug, isLbugReady, } from '../../../core/lbug/pool-adapter.js';
...
export class StateManager {
    ...
    async refreshRepos() {
        ...
        for (const entry of entries) {
            ...
            if (kuzu.found && kuzu.needsReindex) {
                logger.error(`GitNexus: "${entry.name}" has a stale KuzuDB index. Run: gitnexus analyze ${entry.path}`);
            }
            const handle = {
    ...
    async maybeWarnSiblingDrift(handle) {
        ...
        if (match.match !== 'sibling-by-remote' || !match.entry || !match.cwdGitRoot ||
