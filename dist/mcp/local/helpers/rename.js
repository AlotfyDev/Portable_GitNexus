...
export async function executeRename(repo, params) {
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (lines[i].includes(oldName)) {
                    addEdit(ref.filePath, i + 1, lines[i].trim(), lines[i].replace(new RegExp(`\\b${escapeRegex(oldName)}\\b`, 'g'), new_name).trim(), 'graph');
                    graphEdits++;
                    break;
                }
            }
