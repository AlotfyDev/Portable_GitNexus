...
export const analyzeCommand = async (inputPath, options) => {
    ...
    if (inputPath) {
        ...
        // --source-path CLI flag overrides config source_path
        repoPath = path.resolve(options.sourcePath);
    }
    else if (config?.source_path) {
        // Portable config source_path — resolved relative to cwd
        repoPath = path.resolve(process.cwd(), config.source_path);
    }
