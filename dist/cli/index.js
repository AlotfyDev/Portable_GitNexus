...
program.name('gitnexus').description('GitNexus local CLI and MCP server').version(getVersion());
program
    .command('setup')
...
program
    ...
    .option('--source-path <path>', 'Source directory to analyze (overrides config source_path, alternative to positional [path])')
    .option('--output-path <path>', 'Output path for .gitnexus/ knowledge graph (overrides config output_path)')
...
program
    .command('config')
    .description('View or modify portable configuration')
    .argument('[action]', 'Action: view, get, or set (default: view)')
    .argument('[key]', 'Config key path (e.g. "source_path" or "server.port")')
    .argument('[value]', 'Value to set (for set action)')
    .action(createLazyAction(() => import('./config.js'), 'configCommand'));
program
    .command('clean')
